...
**Задача:** Предоставить класс, способный записывать операции покупки и продажи клиента и отображать информацию в соответствии с критериями приемки. Для простоты – одновременно он должен работать только для одного пользователя. Помните, что цена акций постоянно меняется, поэтому она не должна быть зарадкожена, а должна передаваться в тесте.
